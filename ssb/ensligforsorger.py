...
Produsere csv fil med data fra database:
    ...
    Input parameter: Ingen.
                     Kommandolinje til å starte produsering er python ensligforsorger.py.
"""
...
def generate_csv():
    ...
    print('Produsere csv fil for EF starter ', datetime.datetime.now())
    file_path = "S350_EF_STONAD_SSB_2025.csv"
    with open(file_path, 'w') as f: # with open metode oppretter en ny fil om det ikke finnes, ellers overskriver eksisterende fil
        ...
        write_header=True
        query = "select * from vfam_ef_stonad_ssb_2025"
        df = pd.read_sql(query, con=connection)
        df.to_csv(file_path, index=False, sep=';', encoding='utf-8', header=write_header, date_format='%Y%m%d')
    print('Produsere csv fil for EF er fullført ', datetime.datetime.now())

    print('Produsere csv fil for arena starter ', datetime.datetime.now())
    file_path = "S350_EF_STONAD_ARENA_SSB_2025.csv"
    with open(file_path, 'w') as f: # with open metode oppretter en ny fil om det ikke finnes, ellers overskriver eksisterende fil
        # Skilletegn er semikolon
        write_header=True
        query = "select * from vfam_ef_stonad_arena_ssb_2025"
        df = pd.read_sql(query, con=connection)
        df.to_csv(file_path, index=False, sep=';', encoding='utf-8', header=write_header, date_format='%Y%m%d')
    print('Produsere csv fil for arena er fullført ', datetime.datetime.now())
